...
<h4 class="widget-title">Generate Bill List</h4>
</div>
...
<div class="form-group">
    ...
    <div class="col-sm-3">
    <select tabindex="3" id="cat_id" name="cat_id" class="col-xs-10 col-sm-12">
...
<div class="col-sm-12">
    <button  tabindex="4" style="margin-left:20%;" class="btn btn-info" type="button" id="btn_show" onclick="GetList();return false;">
...
function GetList() {
    data = "to="+$('#to').val()+"&from="+$('#from').val()+"&cat_id="+$('#cat_id').val();
    $(".loading").show();
    $.ajax({
        url: "<?php echo base_url();?>index.php/transactionController/generate_bill_list",
        type: "GET",
        data: data,
        cache: false,
...
$(document).ready(function () {
    ...
    $('#btn_print,#btn_mail').click(function () {
        ...
        if ($("#userform").valid() == true) {
            ...
            $('.loading').show();
            $.ajax({
                url: "<?php echo base_url();?>index.php/transactionController/generate_bill_print_preview",
                type: "POST",
                data: data +'&list=list',
                dateType: "html",
                cache: false,
...
function DeleteRecord(ID) {
  ...
  function exportExcel(){
   ...
   to=$('#to').val();
   $.ajax({
        url: "<?php echo base_url();?>index.php/transactionController/dispatch_query?from="+$('#from').val()+'&to='+$('#to').val(),
        type: "POST",
        data: data,
        success: function (html) {
           window.open('<?php echo base_url();?>index.php/transactionController/dispatch_excel/'+from+'/'+to+'/'+html,'_blank');
        }
    });
    return false;
  }
