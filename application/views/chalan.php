   ...
   <div id="data-list">
    ...
    <div id="data-form" style="display:none;">
		...
		<table id="TblRpt" class="table" style="margin-bottom:0px;border:0px;width:540px;">
		 ...
		 <tbody id="TblRptBody">
		  <tr>
           <td style="border:0px;"><input tabindex="5" type="text" id="txt_item" class="txt_item item" onkeyup="getItemAutoCompt(this);" onblur="GetQtyBag();return false;" list="0"/><input type="hidden" id="item_id" /><input type="hidden" id="order_id"/></td>
            
           <td style="border:0px;"><input style="width:40px;" tabindex="6" type="text" id="txt_qtymt" class="txt_cls"/>
           ...
           <input  type="hidden" id="txt_item_remark" class="txt_cls"/>
           <input  type="hidden" id="txt_unit" class="txt_cls"/>
		...
		<div class="form-group">
		<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> LR Entry</label>

			<div class="col-sm-4">
				<select tabindex="13" name="truckno" id="truckno">
		...
		<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Loading Person Name</label>
			...
			<div class="col-sm-4">
				<input type="text" tabindex="14" name="loading_person_name" id="loading_person_name" placeholder="Loading Person Name" class="col-xs-11"/><!-- col-xs-10 col-sm-12 -->
			</div>	
		...
		<script type="text/javascript">
            ...
            function GetFreight(id){
 		    ...
 		    function GetList() {
		        ...
		        $(".loading").show();
		        $.ajax({
		            url: "<?php echo base_url();?>index.php/transactionController/sales_list",
		            type: "GET",
		    ...
		    function GetItemList(ID) {
		        ...
		        $(".loading").show();
		        $.ajax({
		            url: "<?php echo base_url();?>index.php/transactionController/chalan_get_item",
		            type: "GET",
		        ...
		        if(company_id=='' || company_id==0){
                   alert('Unexpected Error ! Login Again .');
		        	...
		        	//  if(rowCount>1){
		                ...
		                $.ajax({
		                    url: "<?php echo base_url();?>index.php/transactionController/sales_save/tbl_trans1/id",
		                    type: "POST",
		                    data: data,
		                    cache: false,
		                    success: function (html) {
...
//            	cRow=$(param).parent().parent();
            ...
            function DeleteRecord() {
		        ...
		        if (r == true) {
		            $('.loading').show();
		                $.ajax({
		                    url: "<?php echo base_url();?>index.php/transactionController/chalan_delete/"+ID,
		                    type: "POST",
		                    data: {ID: ID},
		                    cache: false,
		                    success: function (html) 
...
//	            	cRow=$(param).parent().parent();
			  ...
			  function deleteRow(row)
				...
				function AddItemsToTable() {
		            ...
		            qty_html += "<input type='hidden' name='item_remark[]' value='" + freight + "' class=''/>";
		            qty_html += "<input type='hidden' name='unit[]' value='" + freight + "' class=''/>";
		            qty_html += qtymt;
		    ...
		    //     function getbalqty(param,party_id,item_id)
		    //     {		    	
			   //      var url = "<?php echo base_url();?>index.php/master_general/getorderbalqty?party_id=" + party_id+"&item_id="+item_id;
			   //      $.get(url, function (data) {
			   //      	//alert(data);
			   //          var report_obj = JSON.parse(data);
						// $('#txt_bal_qty').val(report_obj.bal_qty);			
