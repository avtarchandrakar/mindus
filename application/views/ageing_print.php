...
<body>
    ...
    echo '<center>';
    echo '<h3 style="font-size:15px">Ageing Report</h3>';
    echo '<p style="font-size:14px">District : '.$district.', Date : '.$to.'</p>';
    echo '</h4>';
    echo '</center>';
      ...
      $query=$this->db->query("select m.id,m.name,m.opbalance,
                ...
                from m_ledger m where m.district='$district' group by m.id,m.name order by m.name");
      $result=$query->result();
