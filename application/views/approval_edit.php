	...
	<div id="data-form">
		...
		<div class="form-group">
			...
			<div class="col-sm-3">
				...
				<input tabindex="1" type="text"  name="cdate" id="cdate" data-rule-required="true"  placeholder="Date" class="col-xs-10 col-sm-12 date-picker cdate hasDatepicker" list="0"/>					
			
			</div>
			<label class="col-sm-2 control-label no-padding-right " for="form-field-1"> CPO No</label>
			
			<div class="col-sm-3" >
					<input type="text" tabindex="2" name="cpoapp_no" id="cpoapp_no" class="col-xs-10 col-sm-12" placeholder="CPO No." data-rule-required="true"/>
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-3">
				<input tabindex="3" type="text" readonly name="lname" id="lname" autocomplete="off" placeholder="Ledger Name" class="ledgerinfo col-xs-10 col-sm-12" list="0" /><input type="hidden" id="ledger_id" name="ledger_id">
			</div>

			<label class="col-sm-2 control-label no-padding-right " for="form-field-1"> Delivery Period</label>
			
			<div class="col-sm-3" >
					<input type="text" tabindex="4" name="delevery_preiod" id="delevery_preiod" class="col-xs-10 col-sm-12" placeholder="Delivery Period" data-rule-required="true"/>
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Upload Docs</label>
			<div class="col-sm-3">
				<input type='file' tabindex="5"  data-rule-required="true" id='photo' name='photo' class='form-control  col-xs-10 col-sm-12' />
				<input type="hidden" id="filepath" name="filepath" readonly="readonly"/>
			 	...
			 	<span id="uploaddoc"></span>
			</div>

			<label class="col-sm-2 control-label no-padding-right " for="form-field-1"> Remark</label>
			
			<div class="col-sm-3" >
					<input type="text" tabindex="3" name="remark" id="remark" class="col-xs-10 col-sm-12" placeholder="Remark" data-rule-required="true"/>
			</div>
		</div>
...
//		                $('#TblRpt tbody').append(html);
		    ...
		    {       
                ...
                if ($('#userform').validationEngine('validate')) 
                {
                    ...
                    var target="<?php echo base_url();?>index.php/transactionController/approve_save/tbl_trans1/id";
                    $('#userform').ajaxSubmit({url:target,
                            ...
                            success: function (html) 
                            {
		                        ...
		                        if (html > 0) 
		                        {
									...
									if(r==true)
									{
						                window.open("<?php echo base_url();?>index.php/transactionController/jobcard_bill_print/"+html,'_blank');
									}	
		                            if (status.val() == "edit") {
		    ...
		    function GetPreviousRecord(ID) {
		        ...
		        $.get(url, function (data) {
		            ...
		            if (report_obj.Message == "Success") {
		                ...
		                $("#remark").val(report_obj.remark);
		                // alert(report_obj.fullpath);
						...
						{
							$(".uploaded").show();
							$("#uploaddoc").html('<a target="_blank" href="'+report_obj.fullpath+'">Attached</a>');	
						}else
						{
							$(".uploaded").show();
							$("#uploaddoc").html('');	
