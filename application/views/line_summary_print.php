...
<body>
    ...
    echo '<center>';
    echo '<h3 style="font-size:13px">Summary Report</h3>';
    echo '<p style="font-size:12px">Line Group : '.$line_name.', Date : '.$to.'</p>';
    echo '</h4>';
    echo '</center>';
      ...
      $to=date('Y-m-d',strtotime($to));
        ...
        $query=$this->db->query("select m.id,m.name,m.opbalance,m.mobilenosms,m.climit,
                  ...
                  from m_ledger m where  m.line_id=".$line_id." group by m.id,m.name,m.mobilenosms order by m.name");
      $result=$query->result();
