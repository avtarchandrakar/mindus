...
<h4 class="widget-title">Summary Report Godown Wise</h4>
</div>
...
<div class="form-group">
    ...
    <div class="col-sm-3">
         <select tabindex="4" name="cat_id" id="cat_id" class="col-xs-10 col-sm-12" >
...
<div class="form-group">
    ...
    <div class="col-sm-3">
         <select tabindex="5" name="source_id" id="source_id" class="col-xs-10 col-sm-12" >
          <option value="%">All</option>
          <? $query=$this->db->query('select id,name from m_master where type="SOURCE" and company_id='.get_cookie('ae_company_id')); ?>
          <? if($query->num_rows()>0){ ?>
          <? foreach($query->result() as $row){ ?>
          <option value="<?=$row->id?>"><?=$row->name?></option>
          <? }} ?>
...
<div class="col-sm-12">
	<button  tabindex="6" class="btn btn-info" type="button" id="btn_show" style="margin-left:22%;">
		<i class="ace-icon fa fa-eye bigger-110"></i>
		Show
	</button>
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    <button  tabindex="7" class="btn btn-info" type="submit" id="btn_print" onclick="window.print();">
...
<div id="data-list">
<button class="btn btn-success" type="button" id="btn_excel" onclick="exportExcel();">
...
$(document).ready(function () {
    ...
    $('#btn_show').click(function () {
        ...
        if ($("#userform").valid() == true) {
            ...
            $('.loading').show();
            $.ajax({
                url: "<?php echo base_url();?>index.php/transactionController/summary_report_godown_wise",
                type: "POST",
                data: data +'&list=list',
                dateType: "html",
                cache: false,
                success: function (html) {
                    ...
                    for(i=1;i<cntrow+1;i++){
                        ...
                        $('#data-list-table table').fixedHeaderTable('show');
...
function DeleteRecord(ID) {
  ...
  function exportExcel(){
    $(".mytable").table2excel({
        // exclude CSS class
        exclude: ".noExl",
        name: "Summary_Report_Godown_Wise",
        filename:"Summary_Report_Godown_Wise"
      });    
  ...
  function exportPdf(){
    ...
    doc.autoTable(res.columns, res.data, {
        ...
        columnStyles: {
          ...
          doc.save('summary_report_godown_wise.pdf');
  }
</script>
