	...
	<?php if($p_list==1){?>
		...
		<div class="form-group">
			...
			<label class="col-sm-1 control-label no-padding-right" for="form-field-1">Master Category</label>

			<div  class="col-sm-3">
				<?php
					$query=$this->db->query("select id,name from m_master where company_id=". get_cookie('ae_company_id') ."  and type='MASTER CATEGORY' and prefix='YES' order by name");
					echo "<select id='cat_id' name='cat_id' tabindex='2' class='col-xs-10 col-sm-12' data-placeholder='Select Master Category...''>";
					foreach($query->result() as $row)
					{
  ...
  function hidebtn() {
		    ...
		    function GetList() {
		        ...
		        addLoading();
		        $.ajax({
		            url: "<?php echo base_url();?>index.php/master_general/discount_list_party",
		            type: "GET",
		            data: data,
		            cache: false,
		            success: function (html) {
				        ...
				        $('#newsubmit').click(function () {
				                ...
				                $('.loading').show();
				                $.ajax({
				                    url: "<?php echo base_url();?>index.php/master_general/discount_save_party",
				                    type: "POST",
				                    data: data,
				                    cache: false,
				                    success: function (html) {
