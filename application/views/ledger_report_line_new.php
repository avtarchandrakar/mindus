    ...
    <div id="data-form2" style="display:none;">
        ...
        <div class="widget-header"> 
            <h4 class="widget-title">Ledger Report-Multiple</h4>
        </div>
        ...
        <table class="table">
         <tr>
          <td>From</td><td><input tabindex="1" type="text"  name="from" id="from" placeholder="From" class="cdate col-xs-8 col-sm-8" list="0"/></td>
          <td>To</td><td><input tabindex="2" type="text"  name="to" id="to" placeholder="To" class="cdate col-xs-8 col-sm-8" list="0"/></td>
          <td>Show</td><td>
          ...
          <td>Clear Date</td><td>
            <select name="clear_date" tabindex="4" id="clear_date">
                <option>Yes</option>
                <option>No</option>
            </select>
          </td>
        ...
        <div class="form-group">
            <label for="duallist" class="col-sm-3 control-label no-padding-top"> Select Ledger</label>

            <div class="col-sm-12 multi_inst">
                     <select  tabindex="5" multiple="multiple" size="10" id="party_id" name="party_id" class='demo2 col-md-6' >
        ...
        <div class="form-group"><center>
            <button tabindex="6" type="button" id="btn_show" class="btn btn-primary" onclick="ShowPrint();">Show</button>        </center>
        </div>
...
//                        $('#from').focus();
            ...
            function ShowPrint(){
                ...
                report_type=$('#report_type').val();
                clear_date=$('#clear_date').val();
                window.open($('#baseurl').val()+'index.php/transactionController/lm003/'+from+'/'+to+'/'+l_id+'/'+report_type+'/'+clear_date,'_blank');
            }
            //M-1 :
                function GetParty(){
...
//                $('#from').focus();
            ...
            function GetSummary() {
                to=$('#to').val();
                line_id=$('#party_id').val();
                data ="to="+to+"&line_id="+line_id;
                $(".loading").show();
                $.ajax({
                    url: "<?php echo base_url();?>index.php/transactionController/line_summary_array",
                    type: "GET",
                    data: data,
                    cache: false,
                    success: function (html) {
...
//                },500, function(){
                    $('#TblList > tbody  > tr').each(function(i,tr) {
                        cRow=$(this);
                        console.log(i+'  '+$(this).find('#ledger_id').val());
                        $(this).find('#last_col').addClass('loading_print');

                        var l_id=$(this).find('#ledger_id').val();
                        from=$('#from').val();
                        to=$('#to').val();
                        report_type=$('#report_type').val();
                        clear_date=$('#clear_date').val();
                        data = "l_id="+l_id+"&from="+from+"&to="+to+"&report_type="+report_type+"&clear_date="+clear_date;

                        $.ajax({
                            url: "<?php echo base_url();?>index.php/transactionController/lm003_1",
                            async:false,
                            type: "GET",
                            data: data,
                            cache: false,
                            success: function (html) {
                                $("#data-list-ledger").html(html);
