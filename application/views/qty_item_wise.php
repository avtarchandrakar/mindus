...
?>
    ...
    <div id="data-form2" style="display:none;">
        ...
        <div class="widget-header"> 
            <h4 class="widget-title">Quantity Item Wise Sales Summary of Customer</h4>
        </div>
        ...
        <table class="table">
         <tr>
          ...
          <td>
                <select name="cat_id" id="cat_id" class="col-xs-10 col-sm-12" tabindex="4">
                <? 
                      $query=$this->db->query('select m.id,m.name from m_master m where m.type="MASTER CATEGORY" and m.company_id='. get_cookie('ae_company_id') .' group by m.name,m.id order by m.name'); 
                      if($query->num_rows()>0){
...
//                    $("[tabindex='1']").focus();
            ...
            function GetRpt() {
                ...
                ledger_id=$('#ledger_id').val();
                    var url="<?php echo base_url();?>index.php/transactionController/qty_item_wise";
                data ="from="+from+"&to="+to+"&ledger_id="+ledger_id+"&cat_id="+$("#cat_id").val();
                $(".loading").show();
                $.ajax({
                    url: url,
                    type: "GET",
