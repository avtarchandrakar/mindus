...
?>
    ...
    <div id="data-form2" style="display:none;">
        ...
        <input type="hidden" id="p_delete" value="<?=$p_delete?>" />
...
//                    $("[tabindex='1']").focus();
            ...
            function GetRpt() {
                ...
                to=$('#to').val();
                vtype = 'requisition';
                data ="from="+from+"&to="+to+"&vtype="+vtype;
                p_delete=$('#p_delete').val();
                $(".loading").show();

                $.ajax({
                    url: "<?php echo base_url();?>index.php/transactionController/requisition_rpt",
                    type: "GET",
                    data: data, 
                    cache: false,
...
//                        $('#from').select();
                        ...
                        $("input[type='checkbox'].chk").change(function(){
                            ...
                            if(b == 0){
                                $("#del").hide();
                                $(".deleteselected").html("More H");
                            }
                            else
                            {
                                $("#del").show();
                                $(".deleteselected").html("More H ("+b+")");
                            }
                        });
            ...
            function ShowPrint(){
                ...
                window.open($('#baseurl').val()+'index.php/transactionController/requisition_rpt_print/'+from+'/'+to+'/'+vtype,'_blank');
            }
            //M-1 :
                function GetParty(){
...
//                $('#from').focus();
            ...
            function DeleteSelected() {
                ...
                data="checkbox="+myCheckboxes+"&from="+$("#from").val();
                $(".loading").show();
                $.ajax({
                    url: "<?php echo base_url();?>index.php/transactionController/daily_hideselected",
                    type: "POST",
                    data: data,
                    cache: false,
                    success: function (html) {
