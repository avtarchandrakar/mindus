...
<body>
    ...
    foreach($ledger_ids as $ledger_id)
    {
      ...
      echo '</center>';
        ...
        echo '<table border=0 cellpadding=0 cellspacing=0 style="width:100%;font-size:10px;margin-top:-20px;page-break-after: always;">';
        echo '    <thead>';
        echo '        <tr>';
            ...
            foreach($result as $row)
            {
                  ...
                  if($row->vtype=="sales" || $row->vtype=="purchase"  || $row->vtype=="sales return")
                  {
                     ...
                     if($report_type=="Color Wise")
                     {
                      ...
                      {
                        $query1=$this->db->query("select t2.qtymt,t2.rate,g.name as itemname,t2.discount,t2.percent,t2.freight from tbl_trans2 t2, m_item i,m_master g where t2.itemcode=i.id and i.group_id=g.id and  t2.billno=".$row->id." order by t2.id limit 0,1000");
                      }
                           ...
                           if($row1->discount<>0)
                           {
                              if($discount=="")
                              {
                                 ...
                                 $discount=$discount." + " .$row1->discount." ";
                                    ...
                                    if($row->vtype=="sales" || $row->vtype=="purchase return")
                                    {
                                      $bill_amt=$bill_amt+$freight;
                                    }
                                    if($row->vtype=="sales return" || $row->vtype=="purchase")
                                    {
                                      $bill_amt=$bill_amt-$freight;
                                    }
