...
class Order extends MX_Controller {
	...
	public function addnotetoupdate(){
		...
		if($group>0){
				...
				$this->db->where('order_id',$orderid);
				$this->db->where('groupmid',$group);
				$this->db->update('order_menu_kot',$data);
		}else{
	...
	public function orderlist(){
		...
		$data['settinginfo']=$settinginfo;
	    $data['currency']=$this->order_model->currencysetting($settinginfo->currency);
	    $data['allpaymentmode']=$this->order_model->allpaymentmode();
		$data['taxinfos'] = $this->taxchecking();
		$data['module'] = "ordermanage";
		$data['page']   = "orderlist";   
		echo Modules::run('template/layout', $data); 
	...
	public function allorderlist(){
        ...
        $split='<a href="javascript:;" onclick="showsplit('.$rowdata->order_id.')" class="btn btn-xs btn-success btn-sm mr-1" data-toggle="tooltip" data-placement="left" title="Update" id="table-split-'.$rowdata->order_id.'">'.display('split').'</a>&nbsp;&nbsp;';
			...
			if(!empty($rowdata->marge_order_id)){
			 ...
			 $paymentbtn='<button onclick="changepaymentmode('.$rowdata->order_id.','.$rowdata->payment_method_id.','.$rowdata->order_id.')" id="add0" class="btn btn-xs btn-success btn-sm mr-1" data-toggle="tooltip" data-placement="left" title="Change Payment"><i class="fa fa-edit" aria-hidden="true"></i></button>&nbsp;';
			 $paymentbtn=$paymentbtn.'<button onclick="changebilldate('.$rowdata->order_id.',\''.date('d-m-Y',strtotime($rowdata->bill_date)).'\','.$rowdata->order_id.')" id="add2" class="btn btn-xs btn-success btn-sm mr-1" data-toggle="tooltip" data-placement="left" title="Change Bill Date"><i class="fa fa-calendar" aria-hidden="true"></i></button>&nbsp;';
			$scripted ="function changepaymentmode(id){
                $('#add0').modal('show');
			...
			$row[] = $acptreject.$cancelbtn.$update.$details.$margeord.$posprint.$printmarge.$splitp.$paymentbtn;
			$data[] = $row;
		}
		$output = array(
						"draw" => $_POST['draw'],
	...
	public function cancelitem(){
		...
		$allfoods="";
				...
				//kot
				$this->db->where('order_id',$orderid)->where('menu_id',$sitem)->where('varientid',$vaids)->delete('order_menu_kot');
	...
	public function addtocartupdate(){
				...
				if($isgroup==1){
					...
					if(empty($orderchecked)){
						...
						foreach($groupinfo as $grouprow){
										...
										$this->order_model->new_entry_kot($data3);
									}
					}
					else{
						  $groupinfo=$this->db->select('*')->from('tbl_groupitems')->where('gitemid',$pid)->get()->result();
						  foreach($groupinfo as $grouprow){
									 ...
									 $this->db->where('varientid',$grouprow->varientid);
									 $this->db->where('addonsuid',$uaid);
									 $this->db->update('order_menu_kot',$udata2);
									}
			...
			$this->order_model->new_entry_kot($postInfo);
			}
			else{
				$checkcancelitem=$this->order_model->check_cancelitem($orderid,$pid,$sizeid);
				        $adonsarray=explode(',',$addonsid);
				...
				$this->db->where('varientid',$sizeid);
				$this->db->where('addonsuid',$uaid);
				$this->db->update('order_menu_kot',$udata);
	...
	public function itemqtyupdate(){
			...
			$acqty=	$qty-1;
				...
				if($isgroup==1){
							...
							//kot
							$this->db->where('order_id',$orderid)->where('groupmid',$pid)->where('groupvarient',$sizeid)->where('addonsuid',$uaid)->delete('order_menu_kot');
							if(empty($checkcancelitem)){
									$datacancel=array(
									'orderid'			    =>	$orderid,
									'foodid'		        =>	$pid,
				...
				//kot
				$this->db->where('order_id',$orderid)->where('menu_id',$pid)->where('varientid',$sizeid)->where('addonsuid',$uaid)->delete('order_menu_kot');
					if(empty($checkcancelitem)){
									$datacancel=array(
									'orderid'			    =>	$orderid,
									'foodid'		        =>	$pid,
	...
	public function postokengenerateupdate($id,$ordstatus){
		   ...
		   $this->db->where('ordid',$id)->delete('tbl_updateitems');
            $updatetData = array(
				...
				//kot
				$this->db->where('order_id',$id);
				$this->db->update('order_menu_kot',$updatetData);
		...
		if($this->permission->method('ordermanage','read')->access()==FALSE){
			...
			$kid=$this->input->post('kid',true);
			 $allfood_id=explode(",",$allfood);
				...
				$this->db->where('order_id',$orderid);
		        $this->db->where('menu_id',$foodid);
				$this->db->update('order_menu_kot',$updatetready);
	...
	public function savekitchenitem($orderid){
		...
		foreach($result as $single){
				...
				//kot
				$this->db->where('order_id',$orderid);
				$this->db->update('order_menu_kot',$updatetmenu);
			}
		} 
 ...
 public function closecashregister(){
	...
	public function updateorderjs($id){
		...
		/// Other POS Token
		public function otherpostokengenerate($id,$ordstatus){
			$ordstatus=1;
			$saveid=$this->session->userdata('id');
		   $isadmin=$this->session->userdata('user_type');
		   $customerorder=$this->order_model->read('*', 'customer_order', array('order_id' => $id));

		   // echo $customerorder;die();
		   ...
		   $data['iteminfo']       = $this->order_model->customerorder_new($id,$ordstatus);

		   // echo $this->order_model->customerorder_new($id,$ordstatus);die();
		   $data['billinfo']	   = $this->order_model->billinfo($id);
		   $settinginfo=$this->order_model->settinginfo();
		   $data['settinginfo']=$settinginfo;
		   $data['storeinfo']      = $settinginfo;
		   ...
		   $data['module'] = "ordermanage";
		   $data['page']   = "posinvoice_new";  
		   
		  
		  echo $view = $this->load->view('postoken_new', $data,true);
		   //return $view;
		...
		public function synckot(){
			$kotdetais       = $this->order_model->getsynckot();
			
			foreach ($kotdetais as $kot) {
			$id = $kot->order_id;
			$ordstatus = $kot->isupdate;
			$saveid=$this->session->userdata('id');
		   $isadmin=$this->session->userdata('user_type');
		   $customerorder=$this->order_model->read('*', 'customer_order', array('order_id' => $id));
		...
		public function sync(){
			echo $view = $this->load->view('sync');
		}



	public function changepayment(){
		$orderid                 = $this->input->post('ordred_id');
		$payment_id              = $this->input->post('payment_id',true);
		$payment_method_id       = $this->input->post('payment_method_id',true); 
     	$orderinfo = $this->order_model->payment_method_update($orderid,$payment_method_id);
		// $this->permission->method('itemmanage','update')->redirect();
		redirect('ordermanage/order/orderlist');
	   }

	public function changebilldate(){
		$orderid                 = $this->input->post('bill_ordred_id');
		$bill_cdate       = date('Y-m-d',strtotime($this->input->post('bill_cdate'))); 
     	$orderinfo = $this->order_model->bill_cdate_update($orderid,$bill_cdate);
		// $this->permission->method('itemmanage','update')->redirect();
		redirect('ordermanage/order/orderlist');
	   }
}
